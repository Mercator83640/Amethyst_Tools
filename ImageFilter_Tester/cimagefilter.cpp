...
#include "task_wavelet_templates.h"
...
void CImageFilter::focusProjectionXY()
{
    switch(m_pMethod.m_method)
    {
        ...
        //case FOCUS_PROJ_ARITH: Mimg = focusProjectionArith(Mstack, m_width, m_height, m_nbZ); break;
        //case FOCUS_PROJ_WAVELET: Mimg = focusProjectionWavelet(Mstack, m_width, m_height, m_nbZ, info.m_uniform, info.m_resolution); break;
        case FOCUS_PROJ_GRADIENT: focusProjectionXY_Gradient(); break;

        //case FOCUS_PROJ_LAP: Mimg = focusProjectionLaplacian(Mstack, m_width, m_height, m_nbZ, info.m_taille, info.m_threshold); break;
        //case FOCUS_PROJ_WAVELET1: Mimg = focusProjectionWavelet1(Mstack, m_width, m_height, m_nbZ, info.m_diffMin, info.m_taille, info.m_bGomme); break;
...
void CImageFilter::focusProjectionXY_Wavelet_Lib()
{
    ...
    LibHF.InitFocusStackOption(m_pMethod.m_NameOutput, true);
    LibHF.HyperFocusStart() ;
    for ( int i = 1 ; i < MatInputStack.size() ; i++)
    {
            LibHF.PushImageRaw(MatInputStack.at(i));
    }
    LibHF.HyperFocusFinalMerge();
    qDebug() << "Fin = " << LibHF.HyperFocusWaitDone();
...
//    }
    ...
    for(k=0; k<(long)MatInputStack.size(); k++)
    {
        for(i=0; i<tabH; i++)
        {
         ...
         //           qDebug() << "Copy to " <<  j*TileSize_W << " x "<<  i*TileSize_H << " [" << childW << "x"  << childH ;
...
void CImageFilter::focusProjectionXY_Gradient()
{
 ...
 //   qDebug() << "Etablisseent Carte Poids ";
 //   qDebug() << "type carteID" << carteId.type() << " / " <<  CV_8U << " / " << CV_8UC1 ;
...
//                cv::imwrite("d:\\temp\\testsaveima.tif", carteId) ;
 //               qDebug() << "Type apres tthres" << carteTmpotmp.type() << " " << carteId.type();
        carteClasseTmp = cv::Scalar((double)i);

  //      qDebug() << "bitwise 1 entre type " << carteClasseTmp.type() << " " << carteClasse.type() << " " << carteId.type();
//        if ( carteClasseTmp.type() != carteClasse.type() || carteClasseTmp.type() != carteId.type())
//        {
//            qDebug() << "carteClasseTmp Type incoherent" << carteClasseTmp.type() << " " << carteId.type();
//        }
//        if ( carteClasseTmp.rows != carteClasse.rows || carteClasseTmp.rows != carteId.rows)
//        {
//            qDebug() << "carteClasseTmp rows incoherent";
//        }
//        if ( carteClasseTmp.cols != carteClasse.cols || carteClasseTmp.cols != carteId.cols)
//        {
//            qDebug() << "carteClasseTmp cols incoherent";
//        }


//        cv::bitwise_and(carteClasseTmp, carteId , carteClasse );
//        qDebug() << " size " << carteId.rows << " " <<  carteClasseTmp.rows << " " <<  carteId.cols << " " <<  carteClasseTmp.cols ;
//        qDebug() << "type carteID" << carteId.type() << " / " <<  CV_8U << " / " << CV_8UC1 ;
    ...
    {
        unsigned char cl;
        unsigned short nbiter = 1, Uc, U0, U45, U90, U135, *U, t;
                 ...
                 //   qDebug() << "i = " << i << "\tj = " << j ;
                    for(k=0; k<TailleStructU; k++)
                    {
                     //   qDebug() << "\t\tk = " << k ;
                        ...
                        U[k] = 0;
                        //Site central
                    //     qDebug() << carteClasseTmpU.at<short>(j,i) << " " << i << " x " << j;
 ...
 //   qDebug() << "Finalisation Carte Classe";
    *MatOuput = MatInputStack[0].clone();
...
void CContrast::ComputeCDF(double lfCdf[],float histogram[], long HistoSize)
{
    ...
    for( i = 1; i <= m_BBHE_byDecomposeTh; i++)
    {
